...
with DAG(
    'dh_movie_sftp_data_etl_dag_sample',
    # [START default_args]
    # These args will get passed on to each operator
    # You can override them on a per-task basis during operator initialization
    default_args={'retries': 2},
    # [END default_args]
    description='DH MOVIE SFTP DATA DAG Sample',
    # schedule_interval='@weekly',
    schedule_interval=None,
    start_date=pendulum.datetime(2022, 1, 1, tz="UTC"),
    catchup=False,
    tags=['DH MOVIE DATA SFTP DAG Sample'],
) as dag:
    # [END instantiate_dag]
    # [START documentation]
    dag.doc_md = __doc__
    ...
    upload_task = SFTPOperator(
        task_id="upload",
        ssh_hook=sftp_hook,
        local_filepath="./movies.txt",
        remote_filepath="/movies.txt",
        operation="put",
